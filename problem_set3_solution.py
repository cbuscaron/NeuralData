...
def rts_by_targ_ecc(df):
    ...
    results = df.pivot_table(values='rts', index='targ_ecc', aggfunc='mean')
...
def get_ems(df, trial):
    ...
    # t = *** YOUR CODE HERE ***
    rate = int(df['em_time'][trial][1]-df['em_time'][trial][0])
    start = df['stimon'][trial]/rate
    stop = df['response'][trial]/rate

    indices = range(start, stop)
    t = df['em_time'][trial][indices]
    h = df['em_horiz'][trial][indices]
    v = df['em_vert'][trial][indices]
    return t, h, v
...
def plot_ems_and_target(df, trial):
    ...
    plt.plot([start, stop], [tx, tx],'r',[start, stop], [ty, ty], 'g')
...
def get_rate(spk_times, start, stop):
    ...
    n = sum(np.logical_and(spk_times >= start, spk_times < stop))
    rate = (n/window)*1000.
